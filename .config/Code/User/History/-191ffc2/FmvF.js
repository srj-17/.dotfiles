...
function scientificNotation(value, precision) {
    let multiplier = Math.pow(precision);
    return `${value / multiplier}+e`
} 
...
keyContainer.addEventListener('click', (press) => {
    ...
    if (value) {
        ...
        if (numKeys.includes(press.target) && displayValue.length < 9) {
